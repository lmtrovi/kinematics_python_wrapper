 ...
 *  \param plugin_name <package>/<pluginname> of IKfast plugin, e.g. lmtclam_arm_kinematics/IKFastKinematicsPlugin
 */
class kinematics_wrapper
{
	...
	public:
		kinematics_wrapper(std::string node_name, std::string plugin_name)
        {
			int c = 0;
			ros::init(c, NULL, node_name, ros::init_options::NoSigintHandler);

            try {
                ik = loader.createInstance(plugin_name);  
...
/** \brief Defines the Python module */
BOOST_PYTHON_MODULE(kinematics_py)
{
    ...
    //class_<KinematicsType, boost::noncopyable, boost::shared_ptr<kb_callback> >("kinematics_base")
    py::class_<KinematicsType>("Kinematics", py::init<std::string, std::string>())
